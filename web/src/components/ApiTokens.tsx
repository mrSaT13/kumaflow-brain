...
export default function ApiTokens() {
  ...
  async function create() {
    ...
    try {
      ...
      // В этот браузер сохраняем ТОЛЬКО если там пусто (первый токен —
      // иначе веб сам себе закроет API). Токен для чужого клиента НЕ должен
      // угонять браузер админа: иначе веб начнёт ходить под чужим user_id
      // и все чужие вкусы/профили покажут 403 ("...").
      if (!getBrowserToken()) {
        saveBrowser(r.token);
        toast("Токен создан и сохранён в этот браузер ✓ — скопируйте его ещё и в плеер", "ok");
      } else {
        toast("Токен создан ✓ — скопируйте его в плеер. В браузере остался прежний токен.", "ok");
      }
      mutate();
    } catch (e: unknown) {
      toast(fmtErr(e), "err");
